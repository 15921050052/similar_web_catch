...
from trivest_data.dal.check_dao import CheckDao
...
class SimilarWebSpider(BaseSpider):
    ...
    def __init__(self, name=None, **kwargs):
        ...
        self.check_dao = CheckDao(self.name)
    ...
    def start_requests(self):
        ...
        if not self.wait_utils_env_ok():
            self.log_warn(u'环境不可行，退出当前抓取')
            return
        print u'开始执行'

        loop_cache = self.get_loop_cache()
        ...
        self.log_info(u'status：%s' % (status,))
        if not status:
            ...
            self.log_info(u'不存在状态，新建')
        else:
            if status == u'complete':
                self.log_info(u'is_monday：%s，is_tuesday：%s' % (is_monday, is_tuesday))
                if not is_monday and not is_tuesday:
                    # 完成状态下，不是周一，周二，就直接退出不抓
                    self.log_info(u'不是周一周二，不抓取')
                    return
                # 完成抓取的时间
                ...
                if space_day <= 1:
                    # 小于等于1天，说明周一刚抓完
                    self.log_info(u'小于等于1天，说明周一刚抓完')
                    return
        ...
        self.save_loop_cache_file(loop_cache)
        ...
        for src in src_list:
            ...
            hash_code = self.check_dao.get_hash_code(new_url)

            if file_util.file_is_exist(u'html/%s/%s.html' % (new.get(u'dir', ''), hash_code)):
                self.log_info(u'文章已经存在：' + new_url)
                continue

            has_req = True
            self.log_info(u"开始抓取详情：" + new_url)
        ...
        not has_req and self.check_all_save(new, src_list, loop_cache)

    def check_all_save(self, new, src_list, loop_cache):
        # 存储成功之后，判断所有文件是否下载全
        file_name_list = file_util.get_all_file_name(u'html/%s/' % (new.get(u'dir', u''),))
        ...
        if all_save:
            ...
            self.save_loop_cache_file(loop_cache)
            self.log_info(u'一轮全部完成抓取')

    def parse_detail(self, response):
        self.log_info(u'抓取返回')
        ...
        if u'distil_r_captcha.html' in source_url:
            self.log_warn(u'被禁止了，重启路由，直接停止')
            network_util.get_new_ip()
            raise CloseSpider(u'强制停止')
        hash_code = response.meta[u'hash_code']
        self.save_html_file(new.get(u'dir', u''), hash_code, response.body)

        self.check_all_save(new, src_list, loop_cache)
        self.is_running = False
        time.sleep(10)
